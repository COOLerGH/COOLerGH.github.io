(function(){
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Делаем доступным запуск из обработчиков кнопок
    ...
    // Привязка кнопок режимов (если inline-onclick отсутствует)
    ...
    // Enter в поле имени = старт обычного режима
    if (nameInput) {
    ...
    // Если canvas отсутствует — просто оставляем UI
    if (!cvs) return;
    ...
    function toRgbaTrail(colorStr) {
      ...
      // Ожидаем rgb(...). Если браузер вернёт rgba(...) — просто уменьшим альфу.
    ...
    // если тема поменялась — чистим холст, чтобы фон сразу обновился
