(function(){
  ...
  // Режим читаем из URL (приоритет), иначе из localStorage.
  // Важно: резолвим/нормализуем режим ещё раз в runLevel(), чтобы он гарантированно совпадал
  // с тем, что передал стартовый экран (и чтобы не ломались +2 минуты и SC в drag).
  ...
  // Базовые значения для компенсации browser zoom.
  // Важно: devicePixelRatio на desktop меняется при Ctrl+/Ctrl-.
  ...
  // Rod colors (can differ by mode)
  let rodColor = ['#8B4513', '#8B4513'];
  let rodTargetRgb = [null, null]; // cached RGB for comparisons (drag mode)
  ...
  // ===== Drag-mode ring color tweak (select collected ring, then R/G/B) =====
  let selectedCollectedId = null;
  let leftShiftDown = false; // only ShiftLeft decreases RGB
  ...
  let toast = null; // { text: string, until: number }
  ...
  // ===== Secret skip (LOL) =====
  // Layout-independent: uses KeyboardEvent.code (KeyL, KeyO)
  const CHEAT_SEQ = ['KeyL', 'KeyO', 'KeyL'];
  let cheatPos = 0;
  let cheatLastTs = 0;

  // ===== Secret toggle (SC) for Drag mode: show RGB labels on collected rings =====
  ...
  function triggerSkipLevel() {
    ...
    // Mark all required rings as collected so the completion animation shows a full pyramid.
    for (let t = 0; t < towers; t++) {
      for (let i = 0; i < target; i++) {
        ...
        if (ring) {
          ring.collected = true;
          ring.dragging = false;
          // In drag mode completion depends on color match, so align ring colors to rod.
  ...
  function handleCheatKeydown(e) {
    ...
    // --- LOL (skip level) ---
    if (now - cheatLastTs > 2000) cheatPos = 0; // timeout between keys
    cheatLastTs = now;
    ...
    // --- SC (toggle RGB labels; drag mode only) ---
    if (mode !== 'drag') return;
  ...
  // ===== Color helpers (for selected collected ring in drag mode) =====
  function clamp255(v) {
    v = Math.round(v);
  ...
  function adjustSelectedChannel(channel, delta) {
    ...
    // In drag mode, completion also depends on color matching the rod.
  ...
  function findCollectedRingAt(x, y) {
    // only makes sense for single-tower (drag mode). Still safe to compute.
    const towerIndex = 0;
    ...
    // Iterate from top to bottom for natural selection.
  ...
  function updateCollectedDisplay() {
    ...
    if (mode === 'drag') {
      // show target rod color to help player tune rings
  ...
  function startCompletionAnimation() {
    ...
    // очистим возможные флаги от прошлых попыток
    rings.forEach(r => {
      r.returning = false;
      r.returned = false;
      r.returnStart = 0;
  ...
  function updateCompletionAnimation(now) {
    ...
    if (completion.phase === 'jump') {
      ...
      // 0 -> вверх -> небольшой возврат, заканчиваем в «подвисании» -24
      const peak = -70;
      const hover = -24;
      let y;
      if (p < 0.62) {
    ...
    if (completion.phase === 'rodDown') {
      ...
      // кольца остаются «подвисшими», стержень опускается обратно вниз с лёгкой пружиной
      const hover = -24;
    ...
    if (completion.phase === 'ringsReturn') {
      ...
      // обновляем уже «летящие» кольца и помечаем завершённые
      ...
      // если у башни нет активного кольца — запускаем следующее
        ...
        if (!completion.activeId[t]) {
          ...
          if (nextOrder != null) {
            ...
            if (ring) {
              ...
              // возвращаем кольцо обратно на стержень (в стопку), а не «в мир»
  ...
  function initRings() {
    ...
    // Set rod colors for the level
    rodColor = ['#8B4513', '#8B4513'];
    rodTargetRgb = [null, null];

    if (mode === 'drag') {
      // Random target rod color in drag mode
      const rgb = {
    ...
    for (let t = 0; t < towers; t++) {
      for (let i = 0; i < S.ringCount; i++) {
        const step = 20;
        const base = 60;
        const size = base + (S.ringCount - 1 - i) * step; // i=0 самый большой
  ...
  function drawRod(towerIndex) {
    ...
    // Colors:
    // - drag: random rod color
    // - two: right rod is gray, left is brown
    // - normal: brown
    if (mode === 'drag') {
  ...
  function drawCollectedRings(towerIndex) {
    ...
    for (let i = 0; i < collected[towerIndex]; i++) {
      ...
      // Во время анимации возврата:
      // - returning: кольцо рисуется отдельно (интерполяцией) в drawReturningRings
      // - returned: кольцо уже «село» на стержень и рисуется без смещения
      // - иначе: кольцо ещё «подвисает» вместе со стопкой (animRingsOffsetY)
      if (ring.returning) continue;
      ...
      // highlight selected collected ring (drag mode)
      ...
      // show RGB labels (secret SC) for drag mode
  ...
  function drawReturningRings(now) {
    ...
    for (const ring of rings) {
      ...
      // highlight selected collected ring (mainly useful outside completion)
      if (mode === 'drag' && selectedCollectedId && ring.id === selectedCollectedId) {
        ctx.globalAlpha = 1;
        ctx.lineWidth = 3;
        ctx.strokeStyle = 'rgba(255, 215, 0, 0.95)';
  ...
  function setupInput() {
    if (mode === 'drag') {
      cvs.addEventListener('pointerdown', (e) => {
        ...
        // 1) allow selecting already collected ring on the rod
        ...
        // 2) otherwise start dragging a falling ring
        ...
        // Clicking a falling ring clears selection
        selectedCollectedId = null;
      ...
      document.addEventListener('keydown', (e) => {
        ...
        // Color tweak for selected collected ring: R/G/B
        // Hold LEFT Shift to decrease.
  ...
  window.runLevel = function(settings, meta) {
    ...
    // Сразу обновим UI таймера по фактическому лимиту (чтобы не мигало значение из HTML)
